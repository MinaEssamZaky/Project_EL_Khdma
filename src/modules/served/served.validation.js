...
export const addServedValidation = Joi.object({
    ...
    Address2: Joi.string().min(1).max(50).allow('', null), // Allow empty or null
   ...
   dayOff: Joi.array().items(Joi.string().valid("Friday", "Saturday", "Sunday", "Monday", "Tuesday", "Wednesday", "Thursday" )).single().default([]).optional()
});

export const updateServedValidation  = Joi.object({
    firstName:Joi.string().min(2).max(10).required(),
    secName:Joi.string().min(2).max(10).required(),
    familyName:Joi.string().min(2).max(10).required(),
    birthDay: Joi.number().integer().min(1).max(31).required(),
