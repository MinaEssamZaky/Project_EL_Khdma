...
const { firstName,secName,familyName,email, password, phone,birthDate} = req.body;
...
if (existingUser) {
    ...
    const user = await userModel.create({ firstName,secName,familyName,email,password:hashedPassword,phone,birthDate});
...
export const LogIn = handleError(async (req, res, next) => {
    ...
    const token = jwt.sign({ id: User._id, email: User.email }, process.env.TOKEN, { expiresIn: "30d" });
